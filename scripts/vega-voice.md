...
## 🔑 2026-08-17 大發現：簡體餵音法＋隨機性警告

1. **TTS 吃簡體字，發音正確率大幅提升**（單字/句型/練習/答對/麥克風在簡體下多數唸對了）。
   新標準流程：**顯示文字用繁體，餵 TTS 前轉簡體**。生成腳本都要走這條。
2. **但 Anna Su 的中文有隨機性**：同一個詞這次對、下次錯（星星→信心/行星 偶發）。
   best-of-3 + STT 擋不住聲調錯（STT 對聲調不敏感）。**最終仍以 Vega 耳朵為準。**
3. 簡體下仍不穩的字（2026-08-17 實測）：閃卡、字卡、單獨的「關」（一關一關）、動畫、照（照順序）、
   玩一玩、有用、連連看、拼拼看——疊字動詞和 ABB 結構特別容易壞。
4. 「星星」攻防戰結論（2026-08-17）：疊詞星星在句中極不穩（信心/行星），猩猩/惺惺同音調包也治不了；
   最終解法＝**改句式**：句中用「一顆星／越多星」、行尾避免星星收尾。TTS 對疊詞普遍脆弱，寫稿盡量少用。
5. 歡迎流程 11 段已全數上線（welcome-1~6、welcome-step-1~5，AUDIO_V 6），
   已知小瑕疵待精修：welcome-4「照」、welcome-5「星星」尾音、step-2「字卡」、step-3 多處輕微。
   終極解法仍是 Vega 本人錄音（她已排程要錄音標音，可一併錄這幾段）。
