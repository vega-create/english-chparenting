    ...
    <div className={`relative ${className}`}>
      ...
      {title && (
        <div className="absolute -top-3 left-1/2 -translate-x-1/2 z-10 whitespace-nowrap rounded-full bg-gradient-to-b from-purple-500 to-indigo-600 px-4 py-0.5 font-black text-white shadow-lg border-2 border-white/60"
          style={{ fontSize: 'clamp(14px,1.15vw,16px)' }}>{title}</div>
      )}
      ...
      <div className="relative mx-auto px-3 py-[3vh]" style={{ maxWidth: '1200px' }}>
        ...
        <div className="relative w-full mx-auto" style={{ aspectRatio: '1200 / 346', maxWidth: 'min(94%, 560px)' }}>
          ...
          <p className="absolute text-center font-black text-white whitespace-nowrap"
            style={{ left: '24.8%', width: '52.2%', top: '70%', fontSize: 'clamp(14px,1.3vw,19px)', textShadow: '0 1px 2px rgba(55,20,95,.7)' }}>
        ...
        {!user && (
          <div className="mt-4 mx-auto max-w-3xl rounded-2xl bg-white/90 backdrop-blur border-2 border-amber-300 shadow-lg px-4 py-3 flex flex-wrap items-center justify-center gap-x-3 gap-y-2">
            ...
            <p className="font-bold text-amber-900" style={{ fontSize: 'clamp(15px,1.25vw,16px)' }}>
              進度目前只存在這台裝置上
            </p>
            <button onClick={() => { playClick(); signIn('/cabin'); }}
              ...
              style={{ fontSize: 'clamp(15px,1.2vw,15px)' }}>
              家長登入，換手機也保留 →
            </button>
          </div>
        )}
        ...
        {/* ============ 小屋主體（登入與否都看得到）============ */}
          ...
          <div className="mt-5 grid lg:grid-cols-[2.65fr_1fr] gap-3 sm:gap-4 items-start">
            ...
            <div className="relative w-full mx-auto" style={{ aspectRatio: '869 / 1667', maxWidth: 'clamp(190px, 27vw, 330px)' }}>
              ...
              {/* 紫緞帶：名字 */}
              <p className="absolute text-center font-black text-white whitespace-nowrap"
                style={{ left: '10%', width: '76%', top: '43.2%', fontSize: 'clamp(14px,1.15vw,17px)', textShadow: '0 1px 2px rgba(55,20,95,.65)' }}>
                {AVATAR_NAME[avatar]}
              </p>
              {/* 第 1 格：等級 + 稱號 */}
              <div className="absolute text-center" style={{ left: '10%', width: '80%', top: '52%' }}>
                ...
                <p className="font-bold text-amber-700 leading-none mt-[4%]" style={{ fontSize: 'clamp(12px,0.92vw,13px)' }}>{CUR.title}</p>
              </div>
              {/* 第 2～3 格：目前世界（縮圖＋中英島名） */}
              <div className="absolute flex flex-col items-center" style={{ left: '10%', width: '80%', top: '63.5%' }}>
                <p className="font-bold text-amber-700 leading-none" style={{ fontSize: 'clamp(12px,0.9vw,13px)' }}>目前世界</p>
                <div className="flex items-center gap-[5%] mt-[3%] w-full justify-center">
                  ...
                  <div className="min-w-0">
                    <p className="font-black text-amber-900 leading-none whitespace-nowrap" style={{ fontSize: 'clamp(13px,1.1vw,16px)' }}>{CUR.zh}</p>
                    <p className="font-bold text-amber-700/85 leading-none mt-[8%] whitespace-nowrap" style={{ fontSize: 'clamp(11px,0.8vw,12px)' }}>{CUR.en}</p>
                  </div>
                </div>
              </div>
              {/* 第 4 格：EXP 條（數字在條內） */}
              <div className="absolute flex items-center gap-[4%]" style={{ left: '11%', width: '78%', top: '82.5%' }}>
                <span className="font-black text-amber-800 shrink-0" style={{ fontSize: 'clamp(11px,0.85vw,12px)' }}>EXP</span>
                <div className="relative flex-1 rounded-full bg-amber-200 overflow-hidden border border-amber-700/30"
                  style={{ height: 'clamp(15px,1.3vw,20px)' }}>
                  ...
                  <span className="absolute inset-0 flex items-center justify-center font-black text-amber-900 leading-none"
                    style={{ fontSize: 'clamp(11px,0.72vw,11px)' }}>{CUR.xp} / {CUR.xpMax}</span>
                </div>
              </div>
              {/* 底部按鈕槽 */}
              <Link href="/adventure-map" onClick={() => playClick()}
                ...
                style={{ left: '13.8%', width: '72.5%', top: '89.4%', height: '6%', fontSize: 'clamp(13px,1.1vw,16px)' }}>
                🗺️ 查看冒險地圖
              </Link>
            </div>
            ...
            <div className="space-y-3 sm:space-y-4 mx-auto w-full" style={{ maxWidth: 'min(100%, 680px)' }}>
                ...
                <div className="absolute flex flex-col justify-between" style={{ left: '7.5%', width: '32%', top: '19%', bottom: '9.5%' }}>
                    ...
                    <div className="mt-[6%] space-y-[4%]">
                      {[
                        ...
                        <div key={r.t} className="flex items-center gap-[4%]">
                          <span style={{ fontSize: 'clamp(14px,1.25vw,19px)' }}>{r.i}</span>
                          <span className="flex-1 font-bold text-amber-800 whitespace-nowrap" style={{ fontSize: 'clamp(12px,0.95vw,14px)' }}>{r.t}</span>
                          <span className="font-black text-amber-900 whitespace-nowrap" style={{ fontSize: 'clamp(12px,0.95vw,14px)' }}>{r.v}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  <div className="text-center">
                    <div className="flex items-center justify-center gap-[3%]">
                      <span style={{ fontSize: 'clamp(15px,1.35vw,20px)' }}>📅</span>
                      <span className="font-bold text-amber-800" style={{ fontSize: 'clamp(12px,0.95vw,14px)' }}>連續登入</span>
                    </div>
                    ...
                    <p className="leading-none mt-[2%] whitespace-nowrap" style={{ fontSize: 'clamp(12px,1vw,16px)' }}>
                ...
                <div className="absolute flex flex-col justify-center" style={{ left: '54.5%', width: '31.5%', top: '68%', height: '22%' }}>
                  ...
                  <div className="flex items-center gap-[5%] mt-[4%]">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="font-bold text-amber-700 leading-none whitespace-nowrap" style={{ fontSize: 'clamp(11px,0.85vw,13px)' }}>{CUR.zh}</p>
                      <p className="font-black text-amber-900 leading-tight mt-[6%]" style={{ fontSize: 'clamp(13px,1.1vw,16px)' }}>
                    ...
                    <Link href="/courses" onClick={() => playClick()}
                      ...
                      style={{ width: 'clamp(16px,1.9vw,30px)', height: 'clamp(16px,1.9vw,30px)', fontSize: 'clamp(12px,1vw,15px)' }}>›</Link>
                  </div>
                </div>
              </div>
              ...
              <div className="relative w-full" style={{ aspectRatio: '1200 / 412' }}>
                ...
                <p className="absolute left-1/2 -translate-x-1/2 font-black text-white whitespace-nowrap z-10"
                  style={{ top: '3%', fontSize: 'clamp(13px,1.15vw,16px)', textShadow: '0 1px 2px rgba(60,25,100,.6)' }}>成就展示櫃</p>
                ...
                {[...badges].sort((a, b) => Number(b.got) - Number(a.got)).slice(0, 6).map((b, i) => (
                  <div key={b.key} className="absolute flex flex-col items-center justify-center gap-[3%]"
                    ...
                    <p className="text-center font-black leading-tight px-[4%]"
                      style={{ fontSize: 'clamp(11px,0.8vw,12px)', color: b.got ? '#fffbeb' : '#d6c7ae', ...SLOT_TXT }}>{b.got ? b.name : '未解鎖'}</p>
                  </div>
                ))}
              </div>
              <p className="text-center font-black text-white" style={{ fontSize: 'clamp(12px,0.95vw,13px)', textShadow: '0 2px 3px rgba(50,25,0,.85)' }}>
            ...
            <div>
                    ...
                    <>
                      ...
                      <div className="absolute flex flex-col items-center justify-center gap-[4%]"
                        ...
                        <p className="font-black text-amber-900 leading-tight text-center whitespace-nowrap"
                          style={{ fontSize: 'clamp(13px,1.05vw,16px)', opacity: f.soon ? 0.55 : 1 }}>{f.t}</p>
                        ...
                        }`} style={{ fontSize: 'clamp(11px,0.82vw,12px)' }}>{f.soon ? '即將推出' : '去看看'}</span>
                      </div>
                    </>
                  );
                  return f.soon
              ...
              <p className="mt-2.5 text-center font-black text-white" style={{ fontSize: 'clamp(14px,1.2vw,17px)', textShadow: '0 2px 4px rgba(60,30,0,.85)' }}>
            ...
            <div className="space-y-3 sm:space-y-4 mx-auto w-full" style={{ maxWidth: 'min(100%, 420px)' }}>
              ...
              <div className="relative w-full" style={{ aspectRatio: '1170 / 1345' }}>
                ...
                {/* 紫緞帶 */}
                <p className="absolute text-center font-black text-white whitespace-nowrap"
                  style={{ left: '12.9%', width: '73.7%', top: '4%', fontSize: 'clamp(14px,1.2vw,18px)', textShadow: '0 1px 2px rgba(55,20,95,.65)' }}>我的單字櫃</p>
                {/* 側邊層板放書 */}
                {[['📕', 8.5, 20.5], ['📗', 8.5, 45.5], ['📘', 8.5, 70.5], ['📙', 75.6, 20.5], ['📓', 75.6, 45.5], ['📔', 75.6, 70.5]].map(([e, x, y], i) => (
                  <div key={i} className="absolute flex items-end justify-center pb-[2%]"
                    ...
                    <span style={{ fontSize: 'clamp(15px,1.5vw,24px)' }}>{e}</span>
                  </div>
                ))}
                {/* 中央木板 */}
                <div className="absolute flex flex-col items-center justify-center"
                  ...
                  <p className="font-bold text-amber-100/85 leading-none mt-[4%]" style={{ fontSize: 'clamp(12px,0.95vw,14px)', ...SLOT_TXT }}>單字圖鑑</p>
                  ...
                  <Link href="/books" onClick={() => playClick()}
                    ...
                    style={{ fontSize: 'clamp(12px,1vw,15px)' }}>查看全部</Link>
                </div>
              </div>
              ...
              <div className="relative w-full" style={{ aspectRatio: '848 / 900' }}>
                ...
                <p className="absolute text-center font-black text-white whitespace-nowrap"
                  style={{ left: '14%', width: '63.2%', top: '4%', fontSize: 'clamp(14px,1.25vw,19px)', textShadow: '0 1px 2px rgba(55,20,95,.65)' }}>每日小獎勵</p>
                {isVip ? (
                  <div className="absolute flex items-center justify-center font-black text-white"
                    style={{ left: '17.9%', top: '79.7%', width: '63.7%', height: '9.9%', fontSize: 'clamp(14px,1.2vw,18px)', textShadow: '0 1px 2px rgba(20,60,10,.7)' }}>
                    已領取 ⏱ 12:45
                  </div>
                ) : (
                  <button onClick={() => playStar()}
                    ...
                    style={{ left: '17.9%', top: '79.7%', width: '63.7%', height: '9.9%', fontSize: 'clamp(14px,1.25vw,19px)', textShadow: '0 1px 2px rgba(20,60,10,.7)' }}>
                    領取獎勵 ❗
                  </button>
                )}
              </div>
